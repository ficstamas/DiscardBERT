...
def train_model(model: PreTrainedModel, optimizer: Optimizer, lr_scheduler: LRScheduler, dataset: Dataset,
...
def eval_model(model: PreTrainedModel, tokenizer: PreTrainedTokenizer, dataset: DatasetDict, compute_metrics: Callable):
    """
    ...
    train = trainer.evaluate(dataset['train'], metric_key_prefix='eval_train')
    dev = trainer.evaluate(dataset['validation'], metric_key_prefix='eval_dev')
    test = trainer.evaluate(dataset['test'], metric_key_prefix='eval_test')
    return {
        "train": train,
        "dev": dev,
        "test": test
